...
export default function AuditLogPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') applyFilters() }} placeholder="Mã, nội dung mô tả..." className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><input aria-label="Từ ngày" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><input aria-label="Đến ngày" type="date" value={to} onChange={(e) => setTo(e.target.value)} className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><input value={actor} onChange={(e) => setActor(e.target.value)} placeholder="Người thực hiện" className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><input value={actionType} onChange={(e) => setActionType(e.target.value)} placeholder="Loại hành động" className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><input value={entityName} onChange={(e) => setEntityName(e.target.value)} placeholder="Module" className="h-10 rounded-lg border border-slate-200 px-3 text-sm" /><select aria-label="Trạng thái" value={status} onChange={(e) => setStatus(e.target.value as AuditStatus | '')} className="h-10 rounded-lg border border-slate-200 px-3 text-sm"><option value="">Tất cả trạng thái</option><option value="SUCCESS">Thành công</option><option value="FAILED">Thất bại</option><option value="WARNING">Cảnh báo</option></select><button onClick={applyFilters} className="h-10 rounded-lg bg-slate-800 px-4 text-sm font-semibold text-white hover:bg-slate-700">Áp dụng bộ lọc</button></div>
    </section>
